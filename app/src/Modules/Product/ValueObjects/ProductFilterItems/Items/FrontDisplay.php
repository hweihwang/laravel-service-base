...
final class FrontDisplay extends AbstractCmsFilterItem
{
    use TermsTrait;
    ...
    public function toElasticQuery(): SyntaxInterface
    {
        ...
        foreach ($this->queryValue as $value) {
            $productBoolQuery->add('should', new Matching('frontDisplay', $value, null));
        }
        
        $skuBoolQuery = new BoolQuery();
        
        foreach ($this->queryValue as $value) {
            $skuBoolQuery->add('should', new Matching('skus.frontDisplay', $value, null));
        }
